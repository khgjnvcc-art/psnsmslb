...
class SMSClientBase:
    ...
    def extract_code_from_text(text: str) -> Optional[str]:
        ...
        if not text:
            return None
        # يبحث عن الأرقام المتتالية من 4 إلى 8 خانات
...
class GrizzlyClient(SMSClientBase):
    ...
    async def get_number(self, service: str, country: str = "any", max_price: Optional[str] = None) -> NumberResponse:
        """
        طلب رقم جديد (يدعم V1 كافتراضي لأنه متوافق تماماً مع بنية النظام)
        """
        ...
        # تنسيق الاستجابة الناجحة: ACCESS_NUMBER:38496653:66846426435
    ...
    async def get_status(self, activation_id: str) -> StatusResponse:
        ...
        elif res_upper == "STATUS_WAIT_CODE" or res_upper == "STATUS_WAIT_RESEND" or res_upper.startswith("STATUS_WAIT_RETRY"):
    ...
    async def set_status(self, activation_id: str, status: int) -> bool:
        """
        تغيير حالة التفعيل
        -1: إلغاء | 1: جاهز | 3: كود ثاني | 6: إكمال التفعيل | 8: حظر
        """
        ...
        expected_responses = {
            ...
            "3": "ACCESS_RETRY_GET",
            "6": "ACCESS_ACTIVATION",
            "8": "ACCESS_CANCEL" # الحظر يعيد إلغاء أيضاً
        }
    ...
    async def get_prices_v3(self, service: str, country: str) -> Dict:
        """جلب الأسعار والمزودين المتاحين (JSON)"""
        params = {"service": service, "country": country}
        return await self._request("getPricesV3", params=params, is_json=True)
...
class AliSMSClient(SMSClientBase):
    ...
    async def get_number(self, service: str, country: str = "any", max_price: Optional[str] = None) -> NumberResponse:
        """طلب رقم من AliSMS (نفس بروتوكول SMS-Hub القياسي)"""
        params = {"service": service, "country": country}
        if max_price:
            params["maxPrice"] = max_price
