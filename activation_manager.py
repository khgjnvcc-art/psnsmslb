...
class ActivationManager:
    ...
    # ==========================================
    async def start_sniper(self, user_id: str, service: str, country: str, operator: Optional[str] = None):
        """بدء عملية الصيد في مهمة منفصلة"""
        ...
        task = asyncio.create_task(self._sniper_worker(user_id, service, country, operator, sniper_id))
        self.active_snipers[sniper_id] = task

    async def _sniper_worker(self, user_id: str, service: str, country: str, operator: Optional[str], sniper_id: str):
        """العامل الذي يحاول شراء الرقم بشكل متكرر حتى النجاح"""
        logger.info(f"Sniper started for user {user_id} - Service: {service}")
        
        attempt = 0
        while True:
            try:
                attempt += 1
                # نحاول أولاً مع Grizzly (أو يمكن التبديل حسب المنطق المفضل)
                # ملاحظة: نستخدم get_number التي ترجع NumberResponse
                number_data: NumberResponse = await self.grizzly.get_number(service, country)
                
                # إذا نجح الصيد:
                logger.info(f"🎯 Sniper Success! Number: {number_data.phone_number}")
                ...
                self.start_sms_polling(number_data.activation_id, "grizzly")
                
                # 3. إيقاف الصياد
                break

            except (NoNumbersError, NoBalanceError):
                # في حال عدم توفر أرقام، ننتظر قليلاً ثم نحاول مرة أخرى
    ...
    async def _poll_sms_worker(self, activation_id: str, provider: str):
        ...
        try:
            while True:
                ...
                status_data = await client.get_status(activation_id)
                ...
                if status_data.status_code == "OK":
                    ...
                    logger.info(f"✅ SMS Received for {activation_id}")
    ...
    async def cancel_number(self, activation_id: str, provider: str) -> bool:
        """إلغاء الرقم واسترداد الرصيد (Status = -1 أو 8 حسب الموقع)"""
        client = self.grizzly if provider == "grizzly" else self.ali
        # نستخدم الحالة -1 للإلغاء كما هو في أغلب بروتوكولات SMS-Hub
